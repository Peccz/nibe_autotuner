    ...
    def _calculate_separate_metrics(self, device: Device, start_time: datetime, end_time: datetime) -> Tuple[Optional[HeatingMetrics], Optional[HotWaterMetrics]]:
        ...
        for supply_ts, supply_temp in supply_readings:
            ...
            if all(v is not None for v in [outdoor_temp, return_temp, comp_freq]):
                if comp_freq >= self.COMPRESSOR_ACTIVE_THRESHOLD:
                    # Removed COP calculation here to simplify tuple creation
                    # We can recalculate it later or pass it if needed, but tuple structure must match
                    # what _calculate_heating_metrics expects.
                    # _calculate_heating_metrics expects: (supply, return, outdoor, comp_freq) + maybe others?
                    
                    # Let's verify what _calculate_heating_metrics expects:
