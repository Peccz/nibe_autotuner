...
def get_analytics_data():
    ...
    try:
        ...
        def get_series(param_id):
            ...
            readings = session.query(ParameterReading)                .filter(ParameterReading.parameter_id == param.id)                .filter(ParameterReading.timestamp >= start_time)                .order_by(ParameterReading.timestamp).all()
