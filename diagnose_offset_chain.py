...
def diagnose():
    ...
    try:
        # We can't easily instantiate the full agent without valid API keys causing init checks,
        # but we can import the class and test the helper method if we mock what's needed.
        # Or simpler: Just replicate the logic used in the agent.
        ...
        if 'metrics' in locals() and metrics.curve_offset is not None:
