...
def get_status():
    ...
    try:
        ...
        if indoor is None and device:
            indoor = analyzer.get_latest_value(device, '40033')
...
def get_plan():
    ...
    try:
        ...
        # 1. History
        hist_indoor = get_readings('40033')
        hist_outdoor = get_readings('40004')
        hist_gm = get_readings('40941') # Pump GM
        hist_bank = get_readings('VP_GM_BANK') # Bank Balance
        ...
        table_data = []
        for s in schedule:
            is_running = 1 if s.planned_action in ['RUN', 'MUST_RUN'] else 0
            table_data.append({
