...
class AutonomousAIAgentV2(AutonomousAIAgent):
    ...
    def analyze_and_decide(self, hours_back: int = 72, dry_run: bool = True, mode: str = "tactical") -> AIDecision:
        ...
        # 2. Check for blocking tests
        # blocking_test = self._check_for_blocking_test() # Disabled for now
        ...
        # Check freshness of indoor temp
        ...
        if latest_reading:
            ...
            if age_seconds > 2700: # 45 minutes
                ...
                logger.warning(f"Indoor temp is STALE! {age_seconds/3600:.1f} hours old. Value: {latest_reading.value}")
                metrics.avg_indoor_temp = latest_reading.value 
            else:
                metrics.avg_indoor_temp = latest_reading.value
        else:
            is_indoor_stale = True
            logger.warning("No indoor temp reading found!")
        ...
        # Define Emergency Logic based on data freshness
        emergency_logic = ""
...
"""
             ...
             emergency_logic = """
1.  **EMERGENCY COMFORT (Safety Override):**
    -   DISABLED due to STALE indoor temperature data (>2h old). Do not make decisions based on indoor temp. Assume sensors are offline.
"""
...
Indoor Temp Data Stale: {is_indoor_stale}
Current Electricity Price: {self.price_service.get_current_price():.2f} SEK/kWh
...
'''
        ...
        try:
            ...
            # Create decision object for Application
            ...
            # Create sanitized decision object for DB Logging (Floats only)
            log_current = None
            log_suggested = None
            try:
                log_current = float(gm_mode)
            except: pass
            try:
                log_suggested = float(decision_model.suggested_value)
            except: pass

            decision_for_log = AIDecision(
                action=decision_model.action,
                parameter=decision_model.parameter,
                current_value=log_current,
                suggested_value=log_suggested,
                reasoning=f"[Mode: {decision_model.suggested_value}] {decision_model.reasoning}",
                confidence=decision_model.confidence,
                expected_impact=decision_model.expected_impact
            )

            # Log sanitized decision
            self._log_decision(decision_for_log, dry_run=dry_run)
