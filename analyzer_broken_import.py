...
class HeatPumpAnalyzer:
    COMPRESSOR_ACTIVE_THRESHOLD = 20  # Hz

    # Default parameters, Nibe F730 specific
    PARAM_OUTDOOR_TEMP = '40004' # BT1
    PARAM_INDOOR_TEMP = '40033'  # BT50
    PARAM_SUPPLY_TEMP = '40008'  # BT2
    PARAM_RETURN_TEMP = '40012'  # BT3
    PARAM_COMPRESSOR_FREQ = '41778'
    PARAM_HEATING_CURVE = '47007'
    PARAM_CURVE_OFFSET = '47011'
    PARAM_DM_CURRENT = '40009'   # Degree Minutes Current
    PARAM_DM_HEATING_START = '47021' # Degree Minutes Heating Start
    PARAM_CALCULATED_SUPPLY_TEMP = '40067' # Calculated Supply Temp
    PARAM_HOT_WATER_TEMP = '40013' # BT7

    # Empirical COP ranges for Nibe F730 (example values)
    ...
    # Degree Minutes targets
    TARGET_DM_MIN = -250
    TARGET_DM_MAX = -150

    # Other constants
    COMPRESSOR_POWER_AVG_KW = 1.5 # Average electrical power consumption of compressor
    ...
    def get_readings(
        ...
        """
        Get all readings for a specific parameter within a time range
        """
        param = self.session.query(Parameter).filter_by(parameter_id=parameter_id_str).first()
        if not param:
    ...
    def calculate_average(
        ...
        """
        Calculate average value for a parameter over a time range
        """
    ...
    def calculate_metrics(
        ...
        metrics = EfficiencyMetrics(
            ...
            curve_offset=curve_offset or 0.0, # NO FALLBACK HERE ANYMORE
        ...
        for supply_ts, supply_temp in supply_readings:
            ...
            if all(v is not None for v in [outdoor_temp, return_temp, comp_freq]):
                if comp_freq >= self.COMPRESSOR_ACTIVE_THRESHOLD:
                    ...
                    if supply_temp < self.HOT_WATER_TEMP_THRESHOLD: heating_data.append((supply_ts, supply_temp, return_temp, outdoor_temp, comp_freq)) 
                    else: hot_water_data.append((supply_ts, supply_temp, return_temp, outdoor_temp, comp_freq, hw_temp))
